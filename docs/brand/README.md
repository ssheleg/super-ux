...
super-ux has two public text surfaces and no more. `ui` is the installer
CLI — every line a user reads in a terminal. `marketing` is the README,
which is also the npm package page and the GitHub front door. There is no
store listing, no robots.txt and no locale files: the keys are omitted
rather than declared empty, because a declared-but-absent source is worse
than a missing one.
