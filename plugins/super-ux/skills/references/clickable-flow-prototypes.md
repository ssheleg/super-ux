...
Part of **super-ux** — see [system-map.md](system-map.md). This is the operator-facing
PRACTICE: when a clickable preview is worth building, what the operator receives from
it, and what its evidence may and may not claim. The machine-checkable graph beneath
it is a different document — [interactive-flow-prototypes.md](interactive-flow-prototypes.md)
owns the bounded scenario graph and its `IFP-01 … IFP-06` rules, and
[prototyping.md](prototyping.md) owns the decision to reach for one at all.

## Contents

- [Start from the scenario model](#start-from-the-scenario-model)
- [What the operator receives](#what-the-operator-receives)
- [Safe simulation and honest evidence](#safe-simulation-and-honest-evidence)
- [Keep decisions and handoff linked](#keep-decisions-and-handoff-linked)
