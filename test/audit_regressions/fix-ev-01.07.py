#!/usr/bin/env python3
"""FIX-EV-01.02 — the outcome corpus for ux-scenarios (sherlock audit, parent
FIX-EV-01; depends on the family harness of FIX-EV-01.01).

The corpus (evals/cases/ux-scenarios.json) holds a positive (claim
...
outcome-case contract, so ux-scenarios can no longer pass an eval by its name
being picked.
...
CASES = os.path.join(ROOT, "evals", "cases", "ux-scenarios.json")
...
def t_positive_carries_scn_branches():
    c = next(x for x in manifest()["cases"] if "scenario-with-branches" in x["id"])
    assert any((o.get("expect") or "") == "SCN-" for o in c["checks"]["outcome"]), \
        "the positive case does not pin SCN ids/branches"


def t_negative_forbids_loading():
    neg = next(c for c in manifest()["cases"] if "negative" in c["id"])
    assert "ux-scenarios" in neg["checks"]["load_trace"]["expect_not_loaded"]


def t_loading_figma_and_empty():
    m = manifest()
    ld = next(c for c in m["cases"] if "no-fake-loading" in c["id"])
    assert any((o.get("expect") or "") == "no timer" for o in ld["checks"]["outcome"]), \
        "the loading case does not forbid fake loading (UX-10)"
    ...
    em = next(c for c in m["cases"] if "empty-response" in c["id"])
    assert any((o.get("expect") or "") == "no personalization" for o in em["checks"]["outcome"]), \
        "the empty-response case does not forbid fake personalization (UX-10)"
...
def main():
    ...
    case("the positive case carries SCN ids and branches",
         t_positive_carries_scn_branches)
    case("the negative case forbids the skill from loading", t_negative_forbids_loading)
    case("loading/figma/empty-state oracles are pinned", t_loading_figma_and_empty)
